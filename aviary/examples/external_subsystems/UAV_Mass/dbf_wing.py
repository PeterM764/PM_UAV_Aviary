...
from aviary.examples.external_subsystems.UAV_Mass.option_info.materials_database import materials
...
class DBFWingMass(om.JaxExplicitComponent):
    def initialize(self):
        self.options.declare(Aircraft.Wing.Dbf.TYPE, types=str)
        self.options.declare(Aircraft.Wing.Dbf.FOAM_DENSITY, types=float)
        self.options.declare(Aircraft.Wing.Dbf.ROD_DENSITY, types=float)
        self.options.declare(Aircraft.Wing.Dbf.ROD_RADIUS, types=float)
        self.options.declare(Aircraft.Wing.Dbf.ROD_THICKNESS, types=float)

        self.options.declare(**make_units_option(Aircraft.Wing.Dbf.FOAM_DENSITY, 'kg/m**3'))
        self.options.declare(**make_units_option(Aircraft.Wing.Dbf.ROD_DENSITY, 'kg/m**3'))
        self.options.declare(**make_units_option(Aircraft.Wing.Dbf.ROD_RADIUS, 'm'))
        self.options.declare(**make_units_option(Aircraft.Wing.Dbf.ROD_THICKNESS, 'm'))
    ...
    def compute_primal(self, span, root_chord, wetted_area):
        ...
        if type == 'simple':
            ...
            rod_thickness = self.options[Aircraft.Wing.Dbf.ROD_THICKNESS]
            foam_density = self.options[Aircraft.Wing.Dbf.FOAM_DENSITY]
            radius = self.options[Aircraft.Wing.Dbf.ROD_RADIUS]
            rod_density = self.options[Aircraft.Wing.Dbf.ROD_DENSITY]
